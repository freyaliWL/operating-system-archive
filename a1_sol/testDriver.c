...
//    printf("%s\n", __func__);
    ...
    // 1
    CHECK(List_insert_after(l, &one) == 0);
    ...
    // Add item (beyond end)
    // 1 4
    CHECK(List_insert_after(l, &four) == 0);
    ...
    // 9 1 4
    CHECK(List_insert_after(l, &nine) == 0);
    ...
    // 9 1 6 4
    CHECK(List_prev(l) == &one);
    CHECK(List_insert_after(l, &six) == 0);
    CHECK(List_count(l) == 4);
    CHECK(List_prev(l) == &one);

    // Add null in middle
    // 9 1 NULL 6 4
    CHECK(List_insert_after(l, NULL) == 0);
...
void TestInsert() {
    ...
    // Add item with nothing (before start)
    // 1
    CHECK(List_insert_before(l, &one) == 0);
    CHECK(List_count(l) == 1);
    CHECK(List_curr(l) == &one);
    ...
    // Add item (beyond end)
    // 1 4
    CHECK(List_insert_before(l, &four) == 0);
    CHECK(List_count(l) == 2);
    CHECK(List_curr(l) == &four);
    ...
    // Add item (before start)
    // 9 1 4
    CHECK(List_insert_before(l, &nine) == 0);
    CHECK(List_count(l) == 3);
    CHECK(List_curr(l) == &nine);
    CHECK(List_next(l) == &one);
    CHECK(List_next(l) == &four);
    ...
    // 9 6 1 4
    CHECK(List_prev(l) == &one);
    CHECK(List_insert_before(l, &six) == 0);
    CHECK(List_count(l) == 4);
    CHECK(List_next(l) == &one);

    // Add null in middle
    // 9 6 NULL 1 4
    CHECK(List_insert_before(l, NULL) == 0);
    CHECK(List_count(l) == 5);
    CHECK(List_curr(l) == NULL);
    CHECK(List_next(l) == &one);
    CHECK(List_next(l) == &four);
...
void TestNodeRecycling() {
    ...
    CHECK(List_insert_after(lists[1], &six) == -1);
    CHECK(List_insert_before(lists[1], &six) == -1);
...
static TestFn dispatchTable[] = {
    ...
    // List/node recycling
    TestNodeRecycling, TestHeadRecycling
};
